...
func atomicPublishDirectoryBound(stage *stagingDirectory, destination, parent string) error {
	...
	err = unix.RenameatxNp(parentFD, filepath.Base(stage.path), parentFD, filepath.Base(destination), unix.RENAME_EXCL)
	if err == nil {
		return nil
	}
...
// exclusive flag prevents an already-created stage replacement from being
...
func rollbackPublishedDirectory(stage *stagingDirectory, destination, parent string) error {
	...
	defer unix.Close(parentFD)
	if err := unix.RenameatxNp(parentFD, filepath.Base(destination), parentFD, filepath.Base(stage.path), unix.RENAME_EXCL); err != nil {
		if errors.Is(err, fs.ErrExist) {
